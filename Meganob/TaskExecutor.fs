...
let Execute(context: BuildContext, rootTask: BuildTask): Task<IArtifact> =
    // TODO: This should get as much data as possible from the cache.
    // TODO: Tests for teh minimal execution (if we have something in the cache, we don't even execute its dependencies).
    ...
    let processTask (buildTask: BuildTask): Task =
        task {
            try
                let inputValues = buildTask.Inputs |> Seq.map (fun input -> outputs[input])
